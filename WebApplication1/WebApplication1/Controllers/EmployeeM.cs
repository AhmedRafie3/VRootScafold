...
namespace WebApplication1.Controllers
{
    ...
    [ApiController]
    public class EmployeeMController : ControllerBase
    {
        private readonly IMongoCollection<EmployeeM> _branchesCollection;

        public EmployeeMController(IMongoDatabase database)
        {
            _branchesCollection = database.GetCollection<EmployeeM>("EmployeeM");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateBranch([FromBody] EmployeeM branch)
        {
            if (branch == null)
            {
        ...
        // 4. Update a Branch
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBranch(string id, [FromBody] EmployeeM updatedBranch)
        {
            var branch = await _branchesCollection.Find(b => b.Id == id).FirstOrDefaultAsync();
            if (branch == null)
            {
