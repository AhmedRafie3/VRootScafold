...
namespace WebApplication1.Controllers
{
    ...
    [ApiController]
    public class BranchMController : ControllerBase
    {
        private readonly IMongoCollection<BranchM> _branchesCollection;

        public BranchMController(IMongoDatabase database)
        {
            _branchesCollection = database.GetCollection<BranchM>("BranchM");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateBranch([FromBody] BranchM branch)
        {
            if (branch == null)
            {
        ...
        // 4. Update a Branch
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBranch(string id, [FromBody] BranchM updatedBranch)
        {
            var branch = await _branchesCollection.Find(b => b.Id == id).FirstOrDefaultAsync();
            if (branch == null)
            {
