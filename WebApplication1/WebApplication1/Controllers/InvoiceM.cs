...
namespace WebApplication1.Controllers
{
    ...
    [ApiController]
    public class InvoiceMController : ControllerBase
    {
        private readonly IMongoCollection<InvoiceM> _branchesCollection;

        public InvoiceMController(IMongoDatabase database)
        {
            _branchesCollection = database.GetCollection<InvoiceM>("InvoiceM");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateBranch([FromBody] InvoiceM branch)
        {
            if (branch == null)
            {
        ...
        // 4. Update a Branch
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBranch(string id, [FromBody] InvoiceM updatedBranch)
        {
            var branch = await _branchesCollection.Find(b => b.Id == id).FirstOrDefaultAsync();
            if (branch == null)
            {
