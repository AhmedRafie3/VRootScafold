...
namespace WebApplication1.Controllers
{
    ...
    [ApiController]
    public class ProductMController : ControllerBase
    {
        private readonly IMongoCollection<ProductM> _branchesCollection;

        public ProductMController(IMongoDatabase database)
        {
            _branchesCollection = database.GetCollection<ProductM>("ProductM");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateBranch([FromBody] ProductM branch)
        {
            if (branch == null)
            {
        ...
        // 4. Update a Branch
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBranch(string id, [FromBody] ProductM updatedBranch)
        {
            var branch = await _branchesCollection.Find(b => b.Id == id).FirstOrDefaultAsync();
            if (branch == null)
            {
