...
namespace WebApplication1.Controllers
{
    ...
    [ApiController]
    public class InvoiceDetailMController : ControllerBase
    {
        private readonly IMongoCollection<InvoiceDetailM> _branchesCollection;

        public InvoiceDetailMController(IMongoDatabase database)
        {
            _branchesCollection = database.GetCollection<InvoiceDetailM>("InvoiceDetailM");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateBranch([FromBody] InvoiceDetailM branch)
        {
            if (branch == null)
            {
        ...
        // 4. Update a Branch
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBranch(string id, [FromBody] InvoiceDetailM updatedBranch)
        {
            var branch = await _branchesCollection.Find(b => b.Id == id).FirstOrDefaultAsync();
            if (branch == null)
            {
